...
test_that("must supply at least one field", {
  expect_error(new_rschedule(list(), "foo"), "at least one field")
})

test_that("must supply named fields", {
  expect_error(new_rschedule(list(1), "foo"), "named elements")
})

test_that("must be a list", {
  expect_error(new_rschedule(1, "foo"), "must be a list")
})

test_that("class is required", {
  expect_error(new_rschedule(list(x = 1)))
})

test_that("can create rschedule subclasses", {
  x <- new_rschedule(list(x = 1), class = "foobar")
