...
To build on `alma_adjust()`, let's assume that rather than just adjusting existing dates, you actually want to _shift_ the dates first, _then_ perform an adjustment to ensure that you have not landed on an event date. There are two natural ways to perform this shift, captured in `alma_jump()` and `sch_step()`.
...
`sch_step()` is a specialized version of shifting dates that is only applicable when shifting by a set number of days (as opposed to weeks or months). Rather than taking the `alma_jump()` approach by applying the jump all at once, `sch_step()` instead "steps" 1 day at a time, _and applies `alma_adjust()` at every step_. This will result in correct "business logic" of stepping forward by "2 business days" that `alma_jump()` couldn't provide.
