...
#'
#' `rr_on_mday()` recurs on a specific day of the month.
#'
#' @details
#'
#' If the day of the month doesn't exist for that particular month, then it
#' is ignored. For example, if `rr_on_mday(30)` is set, then it will never
...
#' library(magrittr)
#'
#' # When used with a yearly or monthly frequency, `rr_on_mday()` expands the
#' # number of days in the recurrence set.
#' on_yearly <- yearly()
#' on_yearly_mday_1_to_2 <- on_yearly %>% rr_on_mday(1:2)
#'
#' start <- "1999-01-01"
#' end <- "2000-06-30"
#'
#' sch_seq(start, end, on_yearly)
#' sch_seq(start, end, on_yearly_mday_1_to_2)
#'
#' # When used with a daily frequency, `rr_on_mday()` limits the number of
#' # days in the recurrence set.
#' on_daily <- daily()
#' on_daily_mday_1_to_2 <- on_daily %>% rr_on_mday(1:2)
#'
#' length(sch_seq(start, end, on_daily))
#' length(sch_seq(start, end, on_daily_mday_1_to_2))
#'
...
#' # number of days.
#' on_last_of_month <- monthly() %>% rr_on_mday(-1)
#'
#' sch_seq(start, end, on_last_of_month)
#'
...
#' on_last_friday_of_month <- monthly() %>% rr_on_wday("Friday", -1)
#' sch_seq(start, end, on_last_friday_of_month)
#'
#' @export
rr_on_mday <- function(x, mday) {
  validate_rrule(x)
