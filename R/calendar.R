...
calendar_usa_nyse <- function(since = "1970-01-01") {
  ...
  sch <- sch_add_schedule(sch, hldy_martin_luther_king_jr_day(nyse_start_mlk))
  ...
  if (since <= nyse_stop_lincoln) {
    ...
    sch <- sch_add_schedule(sch, sch_lincoln)
  }
  ...
  sch <- sch_add_schedule(sch, hldy_memorial_day(nyse_start_memorial_day))

  sch <- sch_add_schedule(sch, hldy_independence_day(since))

  nyse_start_labor_day <- max(as.Date("1887-01-01"), since)
  sch <- sch_add_schedule(sch, hldy_labor_day(nyse_start_labor_day))
  ...
  if (since <= nyse_stop_columbus) {
    ...
    sch <- sch_add_schedule(sch, sch_columbus)
  }
...
add_nyse_good_friday <- function(sch, since) {
  sch_add_schedule(sch, nyse_good_friday(since))
}
...
nyse_good_friday <- function(since) {
  ...
  if (since >= as.Date("1898-04-08")) {
    ...
    sch <- sch_add_schedule(sch, hldy_good_friday(as.Date("1907-03-30")))
    return(sch)
  }

  sch <- hldy_good_friday(since = since, until = as.Date("1898-04-07"))
  sch <- sch_add_schedule(sch, hldy_good_friday(since = as.Date("1898-04-09"), until = as.Date("1906-04-12")))
  sch <- sch_add_schedule(sch, hldy_good_friday(as.Date("1907-03-30")))
  sch
}
...
add_nyse_election_day <- function(sch, since) {
  ...
  sch_add_schedule(sch, nyse_election_day(since))
}
